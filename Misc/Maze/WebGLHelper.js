...
uniform int u_lightPosInverted;
...
void main()
{
    ...
    if (u_lightPosInverted == 0)
    {
        v_toLight = (worldPos - lightPos).xyz;
    }
    else
    {
...
WebGLHelper.getContext = function()
{
    if(!WebGLHelper.gl)
    {
        ...
        WebGLHelper.colorLocation = WebGLHelper.gl.getUniformLocation(program, "u_color");
        WebGLHelper.invertLightLocation = WebGLHelper.gl.getUniformLocation(program, "u_lightPosInverted");
        ...
        WebGLHelper.color = new Vector3(0.0, 0.0, 0.0);
...
WebGLHelper.setLightLocationInverted = function(inverted)
{
    WebGLHelper.getContext();

    WebGLHelper.gl.uniform1i(WebGLHelper.invertLightLocation, inverted ? 1 : 0);
};

WebGLHelper.updateColor = function()
{
    WebGLHelper.getContext();
    
    var colorArray = WebGLHelper.color.toArray();
    colorArray.push(0.0);
    
    WebGLHelper.gl.uniform4fv(WebGLHelper.colorLocation, colorArray);
};
