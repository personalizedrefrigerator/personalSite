...
var HTMLHelper =
{
    ...
    "colorArrayToString": function(colorArray)
    {
        ...
        {
            isNumber = typeof (colorArray[i]) == "number";
        
            if(isNumber)
            {
                colorString += Math.floor(colorArray[i]);
            }
            else
            {
    ...
    "createCanvasRepresentationOfImage": function(image, resizeToWidth, resizeToHeight)
    {
    
        var canvas = document.createElement("canvas");
        
        // If the image is already loaded and has size,
        if(image.complete && (image.width > 0 || image.height > 0))
        {
            canvas.width = resizeToWidth || image.width;
            canvas.height = resizeToHeight || image.height;
